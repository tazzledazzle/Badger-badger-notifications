# Legacy note: Git forward-merge spike (removed from codebase)

The repository previously contained a Kotlin prototype that used JGit and JavaMail for a **Git release-branch forward-merge notification** workflow. That code was removed in favor of the multi-channel notification platform described in [ARCHITECTURE.md](../ARCHITECTURE.md).

The following text is preserved from the old `README.md` for historical context only.
